...
import 'package:srm/src/the_mind/the_mind_students/data/model/analytic/analytic_item.dart' show AnalyticItem;
...
class _TheMindStudentsPageState extends State<TheMindStudentsPage> {
  ...
  final List<String> teachers = ["Aziz", "Kamol", "Madina"];
  final List<String> courses = ["IELTS", "General English", "Math"];

  // Аналитика
  final List<AnalyticItem> analytics = const [
    AnalyticItem(title: 'Лиды', value: '124', sub: '+12% за месяц', subPositive: true, icon: Icons.people_alt_outlined, iconColor: Color(0xFF6B7FD4)),
    AnalyticItem(title: 'Студенты', value: '850', sub: 'Всего активных', subPositive: null, icon: Icons.school_outlined, iconColor: Color(0xFF2ECC8A)),
    AnalyticItem(title: 'Должники', value: '42', sub: '5% от общего числа', subPositive: false, icon: Icons.warning_amber_rounded, iconColor: Color(0xFFED6A2E)),
    AnalyticItem(title: 'Сумма долга', value: '450,000 Сум', sub: 'К получению', subPositive: null, icon: Icons.account_balance_wallet_outlined, iconColor: Color(0xFF8A9BB8)),
    AnalyticItem(title: 'Группы', value: '36', sub: '8 направлений', subPositive: null, icon: Icons.groups_outlined, iconColor: Color(0xFF1A2233)),
  ];
    ...
    return BlocBuilder<StudentCubit, StudentState>(
      builder: (context, state) {
        ...
        if (state is StudentLoaded) {
          ...
          final pageStudents = filtered.skip((_currentPage - 1) * _perPage).take(_perPage).toList();
            ...
            body: ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label: const Text('Добавить студента', style: TextStyle(color: Colors.white)),
                ...
                FiltersCard(
                  ...
                  onDayTypeChanged: (v) => setState(() { _selectedDayType = v; _currentPage = 1; }),
                  onTeacherChanged: (v) => setState(() { _selectedTeacher = v; _currentPage = 1; }),
                  onCourseChanged: (v) => setState(() { _selectedCourse = v; _currentPage = 1; }),
                  onStatusChanged: (v) => setState(() { _selectedStatus = v; _currentPage = 1; }),
                  onReset: () => setState(() {
                    _search = '';
